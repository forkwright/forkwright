...
```bash
python -m dispatch "98 and 99 in parallel, then 100 and 101"
```
...
**Bad prompts.** The most consequential failure. A prompt with vague acceptance criteria produces a PR that passes QA trivially but does not actually satisfy the intent. A prompt with an incorrect blast radius produces a PR that modifies the right files for the wrong reasons.
